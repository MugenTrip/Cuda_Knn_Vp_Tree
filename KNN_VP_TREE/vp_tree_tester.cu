...
const static char * STR_CORRECT_WRONG[] = {"WRONG", "CORRECT"};
...
{ 
  ...
  while (temp != NULL) { 
    ...
    if ( isInner != temp->isInner ) {
      //printf("distance: %lf compared to median: %lf \n", dist(X, getVP(temp->T), d) , temp->md );
      //printf("some problem with the leaf\n");
      return 0;
    }
...
int verifyTree(vptree *T, double *vp, node **stack, double md, int isInner,
  ...
  if (isInner && getMD(T) > 2*md){
    printf("error inner_md : %lf > 2*md: %lf \n" , getMD(T) ,2*md );
    return 0;
  }
  ...
  if (isInner){
    ...
    if (isValid==0)
      printf("inner but greater than median\n");
  }
  else{
    isValid = dist(vp, getVP(T), d) > md;
    if (isValid==0){
      printf("outer but less tha median %lf  < %lf \n", dist(vp, getVP(T), d) , md);
      printf("Vantage point: %lf getVP: %lf median: %lf idx: %d  \n", vp[0] , *getVP(T) , md, getIDX(T));
    }
  }
...
{
  if(argc!=3){ 
    printf("\nNeed 2 argument. N: number of elements D: dimensions of elements");
    exit(0);
  }
  ...
  if (!strcmp(STR_CORRECT_WRONG[isValid],"WRONG") || !strcmp(STR_CORRECT_WRONG[foundAll],"WRONG"))
    return 1;
  else
    return 0;
